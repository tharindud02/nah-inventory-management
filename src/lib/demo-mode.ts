...
export function getDemoData(apiPath: string): DemoResponse {
  try {
    ...
    console.error(`Demo data not found for path: ${apiPath}`, error);
    return {
      success: false,
...
export function handleDemoMode(request: NextRequest, apiPath: string): NextResponse | null {
  if (!DEMO_MODE) {
    return null;
  }
...
export function createDemoDataFile(apiPath: string, data: any): void {
  try {
    ...
    console.log(`Demo data created at: ${filePath}`);
  } catch (error) {
    console.error(`Failed to create demo data file: ${error}`);
  }
}
